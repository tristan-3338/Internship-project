...
for k in range(n_plots):
    ...
    for i in range(len(plane_values[k])):
        ...
        diff.append((tf_values[k][i] - tb_values[k][i]) - offset[plane_values[k][i]*16+strip_values[k][i]])
    difference_time.append(diff)
...
for i in range(n_plots):
    ...
    max_diff[plane_values[i][0]] = tf_values[i][0]-tb_values[i][0]
    index_max[plane_values[i][0]] = 0

    for k in range(1,len(plane_values[i])):
        if max_diff[plane_values[i][k]] < tf_values[i][k]-tb_values[i][k]:
            max_diff[plane_values[i][k]] = tf_values[i][k]-tb_values[i][k]
