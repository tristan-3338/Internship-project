...
for n in range(4):
    h_list.append(TH2F("h", "h",36,-2,2,36,-1,17))
    h_list_tot.append(TH2F("h", "h",36,-2,2,36,-1,17))
    ...
    for i in range(len(max_i)): #boucle sur les événments
        ...
        for j in x:
            if j == numero_plane: #si hit dans le plan spécifique
                ...
                h_list_tot[numero_plane].Fill(y_regression_time,y_regression_charge)
                if abs((y_regression_time-y_time[numero_plane])) <= treshold_time[numero_plane] and abs((y_regression_charge-y_charge[numero_plane])) <= treshold_charge:
                    h_list[numero_plane].Fill(y_regression_time,y_regression_charge)
    


for n in range(4):
    # Cloner le premier histogramme pour stocker le résultat
    h = h_list[n].Clone("efficiency")
    h.SetTitle("efficiency of my condition")
    ...
    h.Draw("COLZ")
    c.Update()
    c.SaveAs(f"plot_efficiency/plane{n}.png")
